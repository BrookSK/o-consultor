...
class PlanoController
{
     ...
     */
    private function gerarPrioridadesIA(array $plano): array    * GERAÇÃO AUTOMÁTICA — a partir do diagnóstico, cria o plano completo
     * (prioridades + tarefas organizadas em ETAPAS sequenciais) de uma vez,
     * e redireciona direto ao Kanban. Usado pelo botão "Gerar Plano de Ação".
     */
    public function gerarAutomatico(): void
    {
        ...
        try {
            ...
            // 4) Semear métricas iniciais coerentes (para a aba de métricas já nascer útil).
            $this->semearMetricasIniciais($planoId, $diagnostico);
    ...
    /**
     * Cria um conjunto padrão de métricas para o plano acompanhar.
     */
    private function semearMetricasIniciais(int $planoId, array $diagnostico): void
    {
        $padrao = [
            ['nome' => 'Faturamento mensal', 'categoria' => 'financeiro', 'unidade' => 'R$', 'frequencia' => 'mensal', 'direcao' => 'cima'],
            ['nome' => 'Novos leads', 'categoria' => 'leads', 'unidade' => 'leads', 'frequencia' => 'semanal', 'direcao' => 'cima'],
            ['nome' => 'Taxa de conversão', 'categoria' => 'comercial', 'unidade' => '%', 'frequencia' => 'quinzenal', 'direcao' => 'cima'],
            ['nome' => 'Processos documentados', 'categoria' => 'operacional', 'unidade' => '%', 'frequencia' => 'mensal', 'direcao' => 'cima'],
        ];
        foreach ($padrao as $m) {
            try { Plano::criarMetrica($planoId, $m); } catch (Exception $e) { /* segue */ }
        }
    }
