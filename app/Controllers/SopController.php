...
class SopController
{
    ...
    private function salvarEstruturaOrganizacional(int $diagnosticoId, array $empresa, array $estruturaCompleta): int
    {
        try {
            ...
            // Verificar se já existe estrutura para este diagnóstico e removê-la (regeneração)
            $estruturaExistente = Database::queryOne(
                "SELECT id FROM estruturas_organizacionais WHERE diagnostico_id = ? AND empresa_id = ?",
            ...
            if ($estruturaExistente) {
                ...
                foreach ($setoresAntigos as $setorAntigo) {
                    Database::execute("DELETE FROM servicos_setor WHERE setor_id = ?", [$setorAntigo['id']]);
                }
            ...
            // 1. Salvar estrutura organizacional principal
            $sucesso = Database::execute(
                "INSERT INTO estruturas_organizacionais (
                    diagnostico_id, empresa_id, nome_empresa, nicho, macro_categoria,
                    estrutura_completa_json, total_setores, status, criado_em
                ) VALUES (?, ?, ?, ?, ?, ?, ?, 'ativo', NOW())",
                [
                    $diagnosticoId,
                    $empresa['id'],
            ...
            if (!$sucesso) {
                Logger::error('FALHA AO SALVAR ESTRUTURA ORGANIZACIONAL');
                return 0;
            }
            ...
            foreach ($todosSetores as $nomeSetor => $configSetor) {
                ...
                $setorSucesso = Database::execute(
                    "INSERT INTO setores_empresa (
                        ...
                        funcao_principal, total_servicos, status, criado_em
                    ) VALUES (?, ?, ?, ?, ?, ?, ?, 'mapeado', NOW())",
                    [
                        $estruturaId,
                        $empresa['id'],
                ...
                if (!$setorSucesso) {
                    Logger::warning('Falha ao salvar setor', ['setor' => $nomeSetor]);
                    continue;
                }
            ...
            Database::execute(
                "INSERT INTO progresso_hierarquico (
                    estrutura_id, empresa_id, setores_mapeados, setores_total,
                    servicos_mapeados, servicos_total, etapa_atual, criado_em
                ) VALUES (?, ?, ?, ?, ?, ?, 'mapeamento_servicos', NOW())",
                [
                    $estruturaId,
                    $empresa['id'],
                    count($todosSetores),
            ...
            Logger::info('ESTRUTURA ORGANIZACIONAL SALVA COM SUCESSO', [
                'estrutura_id' => $estruturaId,
                'total_setores' => count($todosSetores),
                'total_servicos' => $totalServicosGeral
            ]);
    ...
    /**
     * Mapear tipo de setor para enum válido (core, apoio, estrategico)
     */
    private function mapearTipoSetor(string $tipo): string
    {
    ...
    /**
     * Gerar código único de serviço com contador para evitar duplicatas
     */
    private function gerarCodigoServicoUnico(string $setor, string $servico, int $estruturaId, int $contador): string
    {
        $prefixoSetor = strtoupper(substr(preg_replace('/[^A-Za-z]/', '', $setor), 0, 3));
        return sprintf('SRV-%d-%s-%03d', $estruturaId, $prefixoSetor, $contador);
    }
    ...
    public function listarSopsPorDiagnostico(): void
    {
        ...
        // Buscar estrutura organizacional deste diagnóstico
        $estrutura = Database::queryOne(
            "SELECT * FROM estruturas_organizacionais WHERE diagnostico_id = ? AND empresa_id = ? AND status = 'ativo' ORDER BY criado_em DESC LIMIT 1",
            [$diagnosticoId, $empresa['id']]
        );
        ...
        if (!$estrutura) {
            Logger::info('NENHUMA ESTRUTURA ENCONTRADA - EXIBINDO TELA VAZIA');
            $dados = [
                'diagnostico' => $diagnostico,
                'empresa' => $empresa,
                'setores_organizados' => [],
        ...
        // Buscar setores da estrutura, ordenados por tipo e nome
        $setores = Database::query(
        ...
        foreach ($setores as $setor) {
            ...
            foreach ($todos as $servico) {
                ...
                $statusFinal = 'mapeado';
                if ($temSop) {
                    $statusFinal = 'sop_gerado';
                } elseif ($servico['status'] === 'detalhado') {
...
Responda APENAS com o JSON válido do SOP completo, sem explicações adicionais";
    ...
    /**
     * INICIAR geração de SOP em BACKGROUND (evita timeout do proxy).
     * Responde imediatamente ao navegador e continua o trabalho pesado
     * em segundo plano via fastcgi_finish_request(). O navegador acompanha
     * o progresso chamando statusServicoSop().
    ...
    public function processarServicoCompleto(): void
    {
        ...
        try {
            ...
            echo json_encode([
                'sucesso' => true,
                'sop_id' => $sopId,
                'mensagem' => 'Geração adicionada à fila. Processando...'
            ]);
            exit;

        } catch (Exception $e) {
            Logger::error('ERRO ao enfileirar geração de SOP', [
                'erro' => $e->getMessage(),
...
Gere de 6 a 9 categorias, cada uma com 1 a 3 mensagens. As 4 categorias obrigatórias de relacionamento humano (acolhimento, escuta/compreensão, confirmação de entendimento e despedida) devem SEMPRE estar presentes, somadas às categorias específicas deste serviço. Responda APENAS com o JSON válido.";
    ...
    /**
     * Tenta disparar o processamento da fila em background (best-effort).
     * Não bloqueia nem falha se não conseguir; o cron é o mecanismo garantido.
     */
    ...
    /**
     * Processa UM pedido da fila (uma fase por chamada, ou tudo se chamado via CLI).
     * Pode ser chamado pelo cron (worker/processar_fila.php) ou por requisição interna.
     * Retorna array com o status atualizado.
     */
    public function processarProximoDaFila(): array
    {
        ...
        if (!$servico) {
            Database::execute(
                "UPDATE fila_geracao_sop SET status = 'erro', mensagem = 'Serviço não encontrado', atualizado_em = NOW() WHERE id = :id",
                ['id' => $filaId]
            );
        ...
        try {
            ...
            if (!$resultado['sucesso']) {
                Database::execute(
                    "UPDATE fila_geracao_sop SET status = 'erro', mensagem = :msg, tentativas = tentativas + 1, atualizado_em = NOW() WHERE id = :id",
                    ['msg' => substr($resultado['erro'] ?? 'Erro', 0, 490), 'id' => $filaId]
                );
                $this->marcarSopErro($sopId, $resultado['erro'] ?? 'Erro na geração');
                return $resultado;
            }
            ...
            if ($proximaFase >= 8) {
                ...
                // Avançar fase
                Database::execute(
                    "UPDATE fila_geracao_sop SET status = 'pendente', fase_atual = :fase, mensagem = :msg, atualizado_em = NOW() WHERE id = :id",
                    ['fase' => $proximaFase, 'msg' => $resultado['mensagem'] ?? '', 'id' => $filaId]
                );
            }
            ...
            Logger::error('ERRO ao processar fase da fila', [
                'erro' => $e->getMessage(),
                'fila_id' => $filaId,
                'fase' => $proximaFase,
                'trace' => $e->getTraceAsString()
            ...
            $this->marcarSopErro($sopId, 'Erro interno: ' . $e->getMessage());
            return ['sucesso' => false, 'erro' => $e->getMessage()];
        }
    }

    /**
     * Executa UMA fase específica da geração do SOP.
     * Retorna ['sucesso'=>bool, 'mensagem'=>string, 'erro'=>string|null].
     */
    private function executarFaseSop(int $sopId, int $servicoId, array $servico, int $fase): array
    {
        $diagnostico = Diagnostico::buscarPorId((int) $servico['diagnostico_id']);
        $empresa = Empresa::buscarPorId((int) $servico['empresa_id']);
        if (!$diagnostico || !$empresa) {
            return ['sucesso' => false, 'erro' => 'Diagnóstico ou empresa não encontrados.'];
        }
        ...
        if (empty($detalhamento)) {
            $detalhamento = [
                'servico' => $servico['nome_servico'],
                'setor' => $servico['nome_setor'],
                'objetivo_principal' => 'Executar ' . $servico['nome_servico'] . ' de forma eficiente e padronizada',
                'processos_principais' => ['Preparação', 'Execução', 'Finalização'],
                'recursos_necessarios' => ['Sistemas internos', 'Documentação padrão']
            ];
        }
        ...
        $sopRow = Database::queryOne("SELECT conteudo FROM sops WHERE id = :id", ['id' => $sopId]);
        $conteudo = $sopRow ? (json_decode($sopRow['conteudo'], true) ?? []) : [];
        ...
        if ($fase === 8) {
            ...
            $conteudo['gestao_situacoes_fora_controle'] = [
                'cenarios_criticos_obrigatorios' => $c['gestao_situacoes_criticas']['cenarios_criticos_detalhados'] ?? [],
                'scripts_situacoes_dificeis' => $c['gestao_situacoes_criticas']['scripts_situacoes_especificas'] ?? []
            ];
            ...
            Database::execute(
                "UPDATE servicos_setor SET sop_id = :sop_id, tem_sop = 1, status = 'sop_gerado', sop_gerado_em = NOW(), atualizado_em = NOW() WHERE id = :id",
                ['sop_id' => $sopId, 'id' => $servicoId]
            );
            Database::execute(
    ...
    /**
     * Salva o conteúdo do SOP no banco.
     */
    private function salvarConteudoSop(int $sopId, array $conteudo, string $status): void
    {
        Database::execute(
            "UPDATE sops SET conteudo = :conteudo, status = :status, atualizado_em = NOW() WHERE id = :id",
            ['conteudo' => json_encode($conteudo, JSON_UNESCAPED_UNICODE), 'status' => $status, 'id' => $sopId]
        );
    }

    /**
     * [OBSOLETO - mantido para compatibilidade] Geração completa em background.
     * Não é mais usado; a geração agora é faseada via processarServicoCompleto().
     */
    private function executarGeracaoSopBackground(int $sopId, int $servicoId, array $servico): void
    {
        try {
            $diagnostico = Diagnostico::buscarPorId((int) $servico['diagnostico_id']);
            $empresa = Empresa::buscarPorId((int) $servico['empresa_id']);
            ...
            if (!$diagnostico || !$empresa) {
                $this->marcarSopErro($sopId, 'Diagnóstico ou empresa não encontrados.');
                return;
            }
            ...
            $conteudo = [
                ...
                'status_geracao' => 'processando',
                'total_fases' => 3,
                'versao' => '3.0 - Geração em 3 fases (background)',
                'data_criacao' => date('d/m/Y H:i:s')
            ];
            ...
            $conteudo['resumo_executivo'] = $resumo['resumo_executivo'] ?? '';
            $conteudo['responsaveis'] = $resumo['responsaveis'] ?? [];
            $conteudo['competencias_requeridas'] = $resumo['competencias_requeridas'] ?? [];
            $conteudo['pre_requisitos'] = $resumo['pre_requisitos'] ?? [];
            $conteudo['recursos_necessarios'] = $resumo['recursos_necessarios'] ?? [];
            ...
            $conteudo['scripts_comunicacao'] = $proc['scripts_comunicacao_operacionais'] ?? [];
            $conteudo['indicadores_performance'] = $proc['indicadores_performance_operacionais'] ?? [];
            $conteudo['anexos_referencias'] = $proc['documentacao_operacional'] ?? [];
            if (!empty($proc['recursos_operacionais_necessarios'])) {
                $conteudo['recursos_necessarios'] = $proc['recursos_operacionais_necessarios'];
            }
            if (!empty($proc['pre_requisitos_operacionais'])) {
                $conteudo['pre_requisitos'] = $proc['pre_requisitos_operacionais'];
            }
            ...
            $conteudo['matriz_riscos_servico'] = $crit['matriz_riscos_servico'] ?? [];
            $conteudo['treinamento_gestao_crises'] = $crit['treinamento_gestao_crises'] ?? [];
            ...
            Database::execute(
                ...
                ['conteudo' => json_encode($conteudo, JSON_UNESCAPED_UNICODE), 'id' => $sopId]
            );
            ...
            Logger::error('ERRO na geração background do SOP', [
                'erro' => $e->getMessage(),
                'sop_id' => $sopId,
                'trace' => $e->getTraceAsString()
            ]);
    ...
    /**
     * Atualiza o progresso do SOP durante a geração background
     */
    private function atualizarProgressoSop(int $sopId, array $conteudo, int $fase, string $mensagem): void
    {
        $conteudo['status_geracao'] = 'processando';
        $conteudo['fase_atual'] = $fase;
        $conteudo['mensagem_progresso'] = $mensagem;
        Database::execute(
    ...
    /**
     * Marca o SOP com erro na geração
     */
    private function marcarSopErro(int $sopId, string $mensagemErro): void
    {
        $sop = Database::queryOne("SELECT conteudo FROM sops WHERE id = :id", ['id' => $sopId]);
        $conteudo = $sop ? (json_decode($sop['conteudo'], true) ?? []) : [];
        $conteudo['status_geracao'] = 'erro';
        $conteudo['mensagem_progresso'] = $mensagemErro;
        Database::execute(
            "UPDATE sops SET conteudo = :conteudo, atualizado_em = NOW() WHERE id = :id",
            ['conteudo' => json_encode($conteudo, JSON_UNESCAPED_UNICODE), 'id' => $sopId]
        );
        Logger::error('SOP MARCADO COM ERRO', ['sop_id' => $sopId, 'erro' => $mensagemErro]);
    }

    /**
     * Endpoint de POLLING: retorna o status da geração do SOP.
     */
    public function statusServicoSop(): void
    {
        Auth::proteger();
        header('Content-Type: application/json');
        ...
        if (!$sopId) {
            echo json_encode(['sucesso' => false, 'erro' => 'sop_id não informado.']);
            exit;
        }

        $sop = Database::queryOne(
            "SELECT conteudo, status, TIMESTAMPDIFF(SECOND, atualizado_em, NOW()) as segundos_desde_update FROM sops WHERE id = :id",
            ['id' => $sopId]
        );
        if (!$sop) {
            echo json_encode(['sucesso' => false, 'erro' => 'SOP não encontrado.']);
        ...
        $conteudo = json_decode($sop['conteudo'], true) ?? [];
        $statusGeracao = $conteudo['status_geracao'] ?? 'concluido';
        $segundosDesdeUpdate = (int) ($sop['segundos_desde_update'] ?? 0);
        $mensagem = $conteudo['mensagem_progresso'] ?? '';
...
# REGRA DE DISTINÇÃO ENTRE AÇÃO DE CONCLUSÃO E AÇÃO RECORRENTE (aplica-se a QUALQUER setor, não só TI)
- Toda ação do resumo executivo que representa um EVENTO ÚNICO de entrega/conclusão (verbos como: entregar, enviar, emitir, liberar, disponibilizar, confirmar, finalizar, publicar, aprovar — sempre que descrevem algo que acontece uma vez, no momento em que o serviço é concluído) precisa ter um passo próprio que descreva EXECUTAR essa entrega pela primeira vez.
- Um passo de ACOMPANHAMENTO RECORRENTE do mesmo tema (verbos como: revisar, monitorar, reavaliar, atualizar periodicamente, acompanhar, manter atualizado — qualquer ação com cadência definida: diária, semanal, mensal, 'a cada X dias') NUNCA substitui o passo de entrega/conclusão inicial, mesmo que trate do mesmo assunto. São duas ações distintas: uma acontece uma vez (execução), a outra se repete ao longo do tempo (manutenção). Ambas podem existir no mesmo SOP, mas a existência da segunda não dispensa a primeira.
- Ao revisar a cobertura do resumo executivo, CLASSIFIQUE cada ação prometida como 'evento único' ou 'recorrente' ANTES de verificar se está coberta. Uma ação de evento único só é considerada coberta se existir um passo que a EXECUTE como conclusão — não apenas um passo que a monitore depois de já concluída.
- Exemplos em vários setores (o padrão é o mesmo em todos): jurídico pode prometer 'enviar contrato assinado' e ter só 'revisar contratos vigentes mensalmente'; marketing pode prometer 'publicar a campanha' e ter só 'monitorar performance'; financeiro pode prometer 'emitir a nota fiscal' e ter só 'conferir notas no fechamento do mês'. Em todos, a ação de conclusão continua DESCOBERTA e precisa do próprio passo.
- Aplique esta regra APENAS quando o resumo de fato tiver, ao mesmo tempo, uma entrega/conclusão pontual e uma ação recorrente sobre o mesmo tema. Não force a distinção em serviços onde ela não existe.";
    }
        ...
        try {
            ...
            $servico = Database::queryOne(
                "SELECT 
                    ...
                    ss.detalhamento_completo as detalhamento_json_raw
                 FROM servicos_setor ss
                 LEFT JOIN setores_empresa se ON ss.setor_id = se.id
                 LEFT JOIN estruturas_organizacionais eo ON se.estrutura_id = eo.id
            ...
            $servico = array_merge([
                ...
                'sop_gerado_em' => null,
                'atualizado_em' => null,
                'diagnostico_id' => $servico['diagnostico_id'] ?? 0
            ], $servico);
            ...
            // Buscar SOP relacionado se existir (relação via servicos_setor.sop_id)
            $sop = null;
            if (!empty($servico['sop_id'])) {
                $sop = Database::queryOne(
